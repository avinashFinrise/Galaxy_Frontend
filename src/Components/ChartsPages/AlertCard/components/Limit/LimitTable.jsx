...
const componentInfo = { componentname: 'limitTable', componenttype: 'table' }
const groupby = ['symbol', 'groupname']

export default function LimitTable({ tabelData, updateSetting, updateLimitInput }) {
  const gridRef = useRef()


  const extraFields = { "symbol": "", 'groupname': "", 'userid': "", "clientlimit": "", "clientTotalUsed": "", "warning": "", "playSound": "", "blink": "", }
  const generateColDef = useCallback((key) => {
    ...
    if (key == "playSound" || key == "blink") {
      option["cellRenderer"] = (e) => <CheckBox componenttype="limit" grid={e} update={updateSetting} />
    }

    if (key == "clientlimit") {
      option["editable"] = true
      option["cellEditor"] = 'agTextCellEditor'
      option["valueGetter"] = ({ data }) => data["limit"]["clientlimit"] || data["marginConf"]?.["allowed"] || 0

    }

    if (key == "userid") option["aggFunc"] = params => params.values.join(", ")

    if (key == "clientTotalUsed") {
      option["valueGetter"] = ({ data }) => data.cfqty;
    }
    if (key == "warning") {
      option['filter'] = true
      option["showRowGroup"] = false
      option["valueGetter"] = ({ data }) => {
        return data.clientlimit < data.cfqty ? 1 : 0;
      }
    }
  ...
  return (
    <div className="container-fluid"
      ...
      <AgGridReact
        {...gridProps}
        onCellEditingStopped={e => updateLimitInput("limit", e.colDef.field, e.newValue, e.node.id)}
        ref={gridRef}
